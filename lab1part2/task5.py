...
class Group:
    ...
    def print_top(self):
        if self.students:
            top_students = sorted(self.students, key=lambda x: x.average, reverse=True)[:5]
            print('Top five students:')
            for i in range(len(top_students)):
                print(f'{i + 1}. {top_students[i].name} {top_students[i].surname} - {top_students[i].average} points')
